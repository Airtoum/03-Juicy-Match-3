...
var fourthpiece = load("res://Pieces/Magenta.tscn")
...
var waitforswap = false

export (PackedScene) var background

func _ready():
	if fourthcolor:
		possible_pieces.append(fourthpiece)
	randomize()
...
func generate_pieces():
	for i in width:
		for j in height:
			...
			piece.position += Vector2(0, offset * -4)
			piece.expectedposition = piece.position
			piece.move_piece(Vector2(0, offset * 4), "Fall")
...
func pixel_to_grid(touch_position):
	touch_position.x /= scale.x
	touch_position.y /= scale.y
...
func swap_pieces(column, row, direction):
	...
	first_piece.move_piece(Vector2(direction.x * offset, direction.y * -offset), "Swap")
	other_piece.move_piece(Vector2(direction.x * -offset, direction.y * offset), "Swap")
	waitforswap = true
...
func _process(_delta):
	...
	for i in width:
		for j in height:
			waitforswap = waitforswap or all_pieces[i][j].is_still_moving()
			...
			if(all_pieces[i][j] == null):
				for k in range(j + 1, height):
					if(all_pieces[i][k] != null):
						...
						all_pieces[i][k].move_piece(Vector2(0, (k-j) * offset), "Fall")
						all_pieces[i][k] = null
						break
...
func move_piece(p, position_change):
	var tween = p.get_node("Smoove")
	tween.interpolate_property(p, "position", null, p.positon + position_change, 0.5, Tween.TRANS_BACK, Tween.EASE_IN_OUT, 0)
	tween.start()
